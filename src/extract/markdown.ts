 ...
 * extraction (with front matter) when it found content. Pass null to convert the raw page;
 * sites where Defuddle mangles the output are forced raw upstream (see rewrite.ts).
 */
export async function toMarkdown(env: Env, page: FetchedHtml, defuddle: DefuddleResponse | null): Promise<string> {
	...
	const useDefuddle = defuddle !== null && defuddle.wordCount > 0;
